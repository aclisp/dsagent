...
- 群聊支持文本、图文混排，以及其中引用的文本、图文混排、语音转写、图片和文件；引用中的 mention
  不满足群聊触发条件，但外层只有真实 mention 且引用有效时仍创建 Turn。引用文本和引用语音转写以
  Markdown quote block 进入 Prompt。
- 单聊支持文本、图文混排、独立图片、独立文件和独立语音转写，并支持与群聊相同的引用类型。只有
  当前附件或只有引用附件时，分别使用固定且符合来源语义的附件提示作为 Turn 正文。
- 独立群聊语音、视频、卡片事件、机器人自己的消息和其他不支持的消息类型静默忽略。
...
- 同一实例中的企业微信群聊和单聊都能进入共享 Session；marker 能稳定区分 conversation 和发送者，
  普通 UI 只显示“群聊”或“单聊”。
- 群聊保持外层真实 mention 触发；单聊无需 mention。约定范围内的引用内容、语音转写、图片和文件
  能进入共享 Session，明确引用的产物能作为原生附件发回原 conversation。
