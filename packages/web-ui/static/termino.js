...
export function Termino(terminalSelector, keyCodes, settings) {
  ...
  try {
    ...
    let cancelActiveQuestion = null;

    function termInput(question) {
      return new Promise(function(resolve) {

        /// show the question while waiting for input
        let questionElement = document.createElement('pre');
        questionElement.innerHTML = question;
        terminal_console.appendChild(questionElement);
        ...
        function handleCommandForQuestion(event) {
          ...
          if (event.keyCode == Command_Key) {
            ...
            if (value.length != 0) {
              /// fold the answer into the question line; append() adds a text node, so the value is not parsed as html
              questionElement.append(value)
              scrollTerminalToBottom()
              resolve(value)
            } else {
              resolve()
        ...
        /// Settle the question without keypress input (resolves undefined, like empty input).
        cancelActiveQuestion = function() {
          terminalSelector.querySelector(DEF_SETTINGS.terminal_input).removeEventListener('keypress', handleCommandForQuestion);
          InputState = false;
          cancelActiveQuestion = null;
          resolve()
    ...
    // FUNCTION TO CANCEL THE PENDING QUESTION, IF ANY
    function termCancelInput() {
    ...
    return {
      ...
      cancel_input: termCancelInput, // CANCEL THE PENDING QUESTION (RESOLVES UNDEFINED)
